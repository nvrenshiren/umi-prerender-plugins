...
export default (api: IApi, opts: IOpts) => {
  ...
  if (!config.ssr) {
    throw new Error(
      'config must use { ssr: true } when using umi preRender plugin'
    )
  }
  ...
  api.onBuildSuccessAsync(async () => {
    ...
    const routePaths: string[] = getStaticRoutePaths(routes, _).filter(
      (path: any) => !/(\?|\)|\()/g.test(path)
    )
    ...
    for (const url of renderPaths) {
      ...
      let ssrHtml = diyRender
        ? await diyRender(ReactDOMServer, htmlElement, _getDocumentHandler)
        : ReactDOMServer[
      ...
      if (checkSum) {
        try {
          ...
          ssrHtml = ssrHtml.replace(
            '</head>',
            `<script>window.UMI_PRERENDER_SUM_CODE = "${hashCode}";</script></head>`
          )
        } catch (e) {
      ...
      try {
        ...
        fs.writeFileSync(
          path.join(outputRoutePath, filename),
          `<!DOCTYPE html>${ssrHtml}`
        )
